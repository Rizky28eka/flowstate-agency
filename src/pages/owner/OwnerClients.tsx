...
interface Client {
  id: string;
  name: string;
  status: "Active" | "Onboarding" | "Churned" | string;
  contact: string;
  email: string;
  industry: string;
  avatar: string;
...
const OwnerClients = () => {
  ...
  const getStatusBadge = (status: Client["status"]): string => {
    switch (status) {
      case "Active":
  ...
  const industryData = clients.reduce<Record<string, { name: string; value: number; color: string }>>(
    (acc, client) => {
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(clients.reduce((sum, client) => sum + client.satisfaction, 0) / clients.length).toFixed(1)} / 5.0
            </div>
            <p className="text-xs text-muted-foreground">Based on {clients.length} reviews</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Up 3% from last year</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Increasing Y-o-Y</p>
          </CardContent>
        </Card>
      </div>
            ...
            <ChartContainer className="h-64 w-full" config={{}}>
              <PieChart>
                ...
                <Pie
                  ...
                  label
                >
                  {Object.values(industryData).map((entry, index) => (
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50">
                <tr className="border-b">
                  ...
                  <th className="text-left p-3 font-medium">
                    <Button variant="ghost" onClick={() => handleSort("totalBilled")}>
                      Total Billed <ArrowUpDown className="w-3 h-3 ml-1" />
                    </Button>
                  </th>
                  <th className="text-left p-3 font-medium">
                    <Button variant="ghost" onClick={() => handleSort("satisfaction")}>
                      Satisfaction <ArrowUpDown className="w-3 h-3 ml-1" />
                    </Button>
                  </th>
